...
#define DiffuseOldMap	texture1
#define NormalMap	texture2
#define NormalOldMap	texture3
...
#define TattooDiffuseMap	texture10
#define TattooNormalMap	texture11
...
#define Per_pixel_modifiable_variables face_shading_values

#define Per_pixel_auxiliary_variables face_auxiliary_values
...
PS_OUTPUT_TO_USE main_ps(Pixel_shader_input_type In)
{
	...
	calculate_alpha_face(In , pp_static , pp_modifiable, pp_aux);
	apply_alpha_test(In, pp_modifiable.early_alpha_value);
	calculate_normal_face(In , pp_static , pp_modifiable, pp_aux);
	calculate_albedo_face(In , pp_static , pp_modifiable, pp_aux);
	calculate_specular_face(In , pp_static , pp_modifiable, pp_aux);
	calculate_ao_face_forward(In , pp_static , pp_modifiable, pp_aux);
	calculate_final_color_face_wo_specular(In , pp_static , pp_modifiable, Output);
	accumulate_light_contributions_face(In , pp_static , pp_modifiable, Output);
#ifdef SYSTEM_SHOW_VERTEX_COLORS
