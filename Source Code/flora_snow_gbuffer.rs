...
PS_OUTPUT_GBUFFER main_ps(Pixel_shader_input_type In)
{
	...
	calculate_normal_flora(In , pp_static , pp_modifiable, pp_aux);
	calculate_albedo_flora(In , pp_static , pp_modifiable, pp_aux);
	calculate_specular_flora(In , pp_static , pp_modifiable, pp_aux);
	calculate_ao_flora_deferred(In , pp_static , pp_modifiable, pp_aux);
