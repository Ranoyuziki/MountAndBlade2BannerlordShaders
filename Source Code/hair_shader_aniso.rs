...
PS_OUTPUT_TO_USE main_ps(Pixel_shader_input_type In)
{
	...
	calculate_ao_hair_aniso_forward(In , pp_static , pp_modifiable, pp_aux);
	calculate_final_pbr_hair_aniso(In , pp_static , pp_modifiable, Output);
	accumulate_light_contributions_hair_aniso(In , pp_static , pp_modifiable, Output);
#ifdef SYSTEM_SHOW_VERTEX_COLORS
