...
#define parallax_depth	texture1
#define hull_normal	texture2
#define iris_normal	texture3
#define spec	texture4
#define Heightmap	texture6
#define DecalDiffuseMap	texture7
#define DecalNormalMap	texture8
#define DecalSpecularMap	texture9

#define STANDART_FOR_EYE
...
PS_OUTPUT_TO_USE main_ps(Pixel_shader_input_type In)
{
	...
	calculate_alpha_eye(In , pp_static , pp_modifiable, pp_aux);
	apply_alpha_test(In, pp_modifiable.early_alpha_value);
	calculate_normal_eye(In , pp_static , pp_modifiable, pp_aux);
	calculate_albedo_eye(In , pp_static , pp_modifiable, pp_aux);
	calculate_specular_eye(In , pp_static , pp_modifiable, pp_aux);
	calculate_eye_ao_forward(In , pp_static , pp_modifiable, pp_aux);
	calculate_final_eye(In , pp_static , pp_modifiable, Output);
	accumulate_light_contributions_eye(In , pp_static , pp_modifiable, Output);
#ifdef SYSTEM_SHOW_VERTEX_COLORS
