...
PS_OUTPUT_GBUFFER main_ps(Pixel_shader_input_type In)
{
	...
	calculate_diffuse_ao_factor_standart(In , pp_static , pp_modifiable, pp_aux);
	float occlusion_info = calculate_occlusion_info_for_gbuffer(In, pp_modifiable, pp_aux);
	set_gbuffer_values(Output, pp_modifiable.world_space_normal, pp_modifiable.early_alpha_value, pp_modifiable.albedo_color, pp_modifiable.specularity, occlusion_info, pp_static.view_length);
	set_gbuffer_motion_vector(In, Output);

	set_gbuffer_entity_id(In, Output);

#ifdef SYSTEM_SHOW_VERTEX_COLORS
	#if (my_material_id != MATERIAL_ID_TERRAIN)
		Output.albedo.rgb = get_masked_vertex_color(In.vertex_color.rgba).rgb;
	#endif
#endif
	return Output;
}
