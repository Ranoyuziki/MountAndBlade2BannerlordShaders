...
PS_OUTPUT_TO_USE main_ps(Pixel_shader_input_type In)
{
	...
	calculate_normal_glass(In , pp_static , pp_modifiable, pp_aux);
	calculate_albedo_glass(In , pp_static , pp_modifiable, pp_aux);
	calculate_specularity_glass(In , pp_static , pp_modifiable, pp_aux);
	calculate_diffuse_ao_factor_glass_forward(In , pp_static , pp_modifiable, pp_aux);
	calculate_final_glass(In , pp_static , pp_modifiable, Output);
#ifdef SYSTEM_SHOW_VERTEX_COLORS
